...
def create_erdos_renyi_network(N, k, seed, true_mega_node_bool=False, consp_mega_node_bool=False, dir=False):
    ...
    Args:
        ...
        dir (bool): if True, the graph is directed, default is False
...
def create_2d_crossed_grid(N, seed):
    G = nx.Graph()
    m = int(np.sqrt(N))
    for i in range(m):
        for j in range(m):
            G.add_edge((i, j), ((i + 1) % m, j))
            G.add_edge((i, j), (i, (j + 1) % m))
            G.add_edge((i, j), ((i + 1) % m, (j + 1) % m))
            G.add_edge((i, j), ((i + 1) % m, (j - 1) % m))
    return G
